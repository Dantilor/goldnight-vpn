...
const COPY = {
  ...
  liability: {
    body: (
      <div className="space-y-4 text-[13px] leading-relaxed text-on-surface-variant">
        <p>
          GoldNight VPN предоставляет пользователю технический сервис для организации защищённого подключения и доступа к сети
          Интернет.
        </p>
        <p>
          Пользователь самостоятельно определяет, какие сайты, приложения, платформы и иные ресурсы он посещает, а также какие
          данные передаёт, вводит или сохраняет при использовании устройства и сети Интернет.
        </p>
        <p>
          Администрация сервиса не несёт ответственности за убытки, утрату данных, получение доступа к информации пользователя
          третьими лицами, блокировки, ограничения доступа, действия владельцев сторонних сайтов, приложений, сервисов и иных
          ресурсов, если такие последствия возникли, в том числе:
        </p>
        <p>
          — при переходе пользователя на сторонние, небезопасные, поддельные или скомпрометированные сайты и сервисы;
          <br />
          — при вводе пользователем логинов, паролей, кодов подтверждения, платёжных реквизитов или иных конфиденциальных
          данных на сторонних ресурсах;
          <br />
          — при передаче, публикации, распространении или разглашении пользователем ссылки подключения, конфигурации, QR-кода,
          ключа доступа, данных учётной записи или иных реквизитов доступа третьим лицам;
          <br />
          — при использовании пользователем устройства, программного обеспечения, браузера, приложения или сетевой среды, не
          обеспечивающих достаточный уровень защиты;
          <br />
          — в результате действий третьих лиц, операторов связи, владельцев сайтов, приложений, платформ и иных ресурсов, не
          находящихся под контролем GoldNight VPN.
        </p>
        <p>
          Пользователь обязуется самостоятельно обеспечивать конфиденциальность данных доступа, не передавать ссылку
          подключения, QR-коды, конфигурационные данные и иные реквизиты доступа третьим лицам, а также использовать только
          проверенные и безопасные сайты, приложения и каналы связи.
        </p>
        <p>
          Использование сервиса не заменяет соблюдение пользователем общих мер цифровой безопасности, включая проверку
          подлинности сайтов, защиту устройства, сохранность паролей и контроль передаваемых данных.
        </p>
        <p>
          Переходя в Mini App, оформляя, оплачивая или активируя подписку, пользователь подтверждает, что ознакомился с
          условиями использования сервиса, правилами безопасности и принимает указанные условия использования сервиса.
        </p>
        <p>
          Администрация сервиса несёт ответственность в пределах, предусмотренных применимым законодательством и условиями
...
export function ProfileScreen() {
  ...
  return (
    <div className="gn-page-stack pt-5">
          ...
          {(
            [
              ...
              { key: 'privacy' as const, icon: 'verified_user', label: 'Политика конфиденциальности' },
              {
                ...
                icon: 'shield_lock',
                label: 'Ограничение ответственности и безопасность использования'
              }
            ] as const
          ).map((item, index) => (
            <div key={item.key} className={index < 4 ? 'border-b border-outline-variant/5' : ''}>
              <button
                type="button"
